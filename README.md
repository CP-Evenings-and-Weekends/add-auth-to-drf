...
- Write tests covering the full auth flow (401 without token, 201 on signup, token unlocks protected endpoints).  DRF's `APIClient` has `credentials(HTTP_AUTHORIZATION='Token ...')` for setting the header in tests.
